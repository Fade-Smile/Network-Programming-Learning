...
'''
@Project :Python网络开发
@File    :04-TFTP-Server.py
@Author  :Sunshine
@Date    :14/10/2023 14:41
'''
...
def download(filename, client_ip, client_port):
    ...
    try:
        ...
        error_package = struct.pack('!HH5sb', 5, 5, 'error'.encode('utf-8'),
                                    0)  # H 表示python的Integer转成C的没有符号的short，5s表示把Python中包含5个字符的字符串转出C的字符数组
...
def upload(filename, client_ip, client_port):
    new_filename = "Server_" + filename
    f = open(new_filename, 'ab')
    ...
    while True:
        ...
        if len(recv_data) < 516:
            print("客户端:%s,文件上传完成" % client_ip)
            break

        # Send an ACK to the client
        ack_package = struct.pack('!HH', 4, num)
        s.sendto(ack_package, (client_ip, client_port))
...
def server():
    while True:
        ...
        if operator_code[0] == 1:  # Download request
            print("客户端想下载文件：%s" % file_name)
            download(file_name, client_ip, client_port)
        elif operator_code[0] == 2:  # Upload request
            print("客户端想上传文件：%s" % file_name)
            upload(file_name, client_ip, client_port)
        else:
            print("无效选择.")
        ...
        #         print("客户端想下载文件：%s" % file_name)
        #         download(file_name, client_ip, client_port)
        #     # check for upload requests and call the upload function.
        #     elif operator_code[0] == 2:  # 如果等于2就是上传的请求数据包
        #         print(f"客户端想上传: {file_name}")
        #         upload(file_name, client_ip, client_port)
